...
public class Exp3 {
    ...
    public void displayClusters() {
        ...
        int[] countSize = new int[C];
        int noise = 0, sum_ = 0;
        for (Point3D P : DB) {
            // funny one liner to increment each cluster properly
            noise += (P.label == 0) ? 1 : (countSize[P.label - 1]++) * 0;
        }
        // sort & print sizes
        Arrays.sort(countSize);
        sum_ = noise;
        System.out.print("Cluster sizes: [");
        for (int i = C - 1; i >= 0; i--) {
            System.out.print(countSize[i] + ",");
            sum_ += countSize[i];
        }
    ...
    public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        String[] setting = new String[] { "Point_cloud_3.csv", "1.2", "10" };
        for (int i = 0; i < args.length; i++) {
            setting[i] = setting[i];
        }
        List<Point3D> DB = read(setting[0]);
        // No file name means no runtime
        Exp3 dbs = new Exp3(DB);
        // setting epsilon
        dbs.setEps(Double.parseDouble(setting[1]));
        // setting minpts
        dbs.setMinPts(Integer.parseInt(setting[2]));
        // get the clusters
        dbs.findClusters();
        dbs.displayClusters();
        // save the output
        dbs.save(setting[0].split("\\.")[0] + "_clusters_" + dbs.getEps() + "_" + dbs.getMinPts() + "_"
                + dbs.getNumberOfClusters() + ".csv");
